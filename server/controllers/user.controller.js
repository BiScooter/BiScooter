 ...
 exports.GetHomeScreenInfos=catchAsync ( async (req, res, next) => {
  ...
  exports.ViewTransactionHistory = catchAsync(async(req,res,next)=>{
    ...
    exports.GiveComplaint=catchAsync(async(req,res,next)=>{
      ...
      const {client_id}=req.params;
      await db.query(`INSERT INTO COMPLAINT VALUES(DEFAULT,'${DATE}','${DESCRIPTION}','PENDING','${TYPE}','${client_id}');`);
    ...
    exports.Reserving= catchAsync(async(req,res,next)=>{
      ...
      const {client_id}=req.params;
      if (!User.phoneCheck(COST)||!User.phoneCheck(DURATION)
      ||!User.phoneCheck(KICKOFF_STATION_ID)||!User.phoneCheck(DISTINATION_STATION_ID)||User.phoneCheck(BISCOOT_ID)) {
        return next(new ErrorHandling("Enter numbers only!!!!", 400));
      }
...
exports.Canceling=catchAsync(async(req,res,next)=>{
  ...
  const Check_id_existance=await db.query(`SELECT ID FROM RENTALS WHERE ID='${ID}';`);
...
if(Check_id_existance.rowCount==0)
{
  ...
  res.status(200).send({Message:`RENTAL WITH ID '${ID}' HAS BEEN CANCELLED!`});
}
});
...
exports.OfferHisBike=catchAsync(async(req,res,next)=>{
  ...
  if (!User.phoneCheck(GEARS_NUM)||!User.phoneCheck(WEIGHT)
  ||!User.phoneCheck(RNT_COST)||!User.phoneCheck(SIZE)||User.phoneCheck(PRICE)) {
    return next(new ErrorHandling("Enter numbers only!!!!", 400));
  }

  const {client_id}=req.params;

  const offered_biscoot_id=await db.query(`INSERT INTO BISCOOT VALUES (DEFAULT,'${RNT_COST}','${SIZE}','${PRICE}','${IMG}','${client_id}',null,'0')RETURNING ID;`);
 await db.query(`INSERT INTO BIKE VALUES ('${TYPE_OF_BIKE}','${GEARS_NUM}','${BRAND}','${WEIGHT}','${offered_biscoot_id.rows[0].id}')RETURNING BIKE_ID;`);
res.status(200).send({Message:`Your bike has been inserted successfully with id '${offered_biscoot_id.rows[0].id}'!`});
});
...
exports.OfferHisScooter=catchAsync(async(req,res,next)=>{
  ...
  if (
    ...
    return next(new ErrorHandling("Fill All Fields to Offer your scooter!", 409));
  }
  const {client_id}=req.params;
  if (!User.phoneCheck(BATTERY_CAPACITY)||!User.phoneCheck(RANGE)
  ||!User.phoneCheck(MAX_SPEED)||!User.phoneCheck(RNT_COST)
  ||User.phoneCheck(SIZE)||User.phoneCheck(PRICE)) {
    return next(new ErrorHandling("Enter numbers only!!!!", 400));
  }
  const offered_biscoot_id=await db.query(`INSERT INTO BISCOOT VALUES (DEFAULT,'${RNT_COST}','${SIZE}','${PRICE}','${IMG}','${client_id}',null,'0')RETURNING ID;`);
 await db.query(`INSERT INTO Scooter VALUES ('${BATTERY_CAPACITY}','${RANGE}','${MAX_SPEED}','${offered_biscoot_id.rows[0].id}')RETURNING SCOOTER_ID;`);
res.status(200).send({Message:`Your scooter has been inserted successfully with id '${offered_biscoot_id.rows[0].id}'!`});
});
...
exports.RemoveHisScooter=catchAsync(async(req,res,next)=>{
 ...
 SCOOTER_ID
  ...
  if (
    ...
    return next(new ErrorHandling("Fill All Fields to REMOVE your scooter!", 409));
  }
  const {client_id}=req.params;
...
res.status(200).send({Message:`SCOOTER WITH ID '${SCOOTER_ID} HAS BEEN REMOVED`});
});
...
exports.RemoveHisBike=catchAsync(async(req,res,next)=>{
 ...
 BIKE_ID
  ...
  res.status(200).send({Message:`BIKE WITH ID '${BIKE_ID} HAS BEEN REMOVED`});
});
...
exports.ViewOfferedBikes=catchAsync(async(req,res,next)=>{
   ...
   res.status(200).send({Message:`Offered bikes and Scooters`,Bikes:offeredBikes.rows[0],Scooters:offeredScooters.rows[0]});
});